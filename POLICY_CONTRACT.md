...
- `models.provider`, `models.pipeline_mode`, model IDs, endpoint, timeouts, batch/context/output limits, instructions, and reasoning effort: parsed; consumed by model construction or local prompt/model limits. The actionable `one_pass` and `two_call` modes are restricted to explicit Hermes CLI connectors (`hermes-openai-codex` or `github-copilot`); the legacy `two_stage` mode remains non-production.
